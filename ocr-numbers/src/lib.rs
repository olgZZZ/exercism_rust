...
pub fn convert(input: &str) -> Result<String, Error> {
    ...
    if raw_lines.len() % 4 != 0 {
        return Err(Error::InvalidRowCount(raw_lines.len()));
    }
    if raw_lines.iter().any(|line| line.len() % 3 != 0) {
        return Err(Error::InvalidColumnCount(raw_lines[0].len()));
    }

    let mut result: Vec<String> = Vec::new();
    for t in (0..raw_lines.len()).step_by(4) {
...
fn match_line(logical_line: Vec<Vec<char>>) -> String {
    ...
    for i in (0..logical_line[0].len()).step_by(3) {
